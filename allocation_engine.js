...
function applyRules(rec) {
  ...
  // 5. Recycle (Grade E)
  if (rec.normalizedGrade === 'E') {
    return { result:'Recycle', reason:'Recycle: Grade E (not eligible for CRN/Refurbish)' };
  }

  // 6. Auction
  return { result:'Auction', reason:'Auction: does not meet CRN/Refurbish criteria' };
}

function checkCRN(rec) {
  if (!['A','B','C','D'].includes(rec.normalizedGrade)) return {pass:false};

  if (rec.modelType === 'Bar') {
    const N = getNModel(rec.series);
    if (N !== null) {
      const lo = N - 3, hi = N - 1;
      if (rec.seriesNum >= lo && rec.seriesNum <= hi) {
        return { pass:true, reason:`CRN: Bar type, ${rec.series}${hi}~${rec.series}${lo}, Grade ${rec.normalizedGrade}` };
      }
    }
  } else if (rec.modelType === 'FF') {
    const N = getNModel('Z');
    if (N !== null) {
      const lo = N - 2, hi = N;
      if (rec.seriesNum >= lo && rec.seriesNum <= hi) {
        return { pass:true, reason:`CRN: FF type, FF${hi}~FF${lo}, Grade ${rec.normalizedGrade}` };
      }
    }
  }
  return {pass:false};
...
function checkRefurbish(rec) {
  const origGrade = String(rec.grade).trim().toUpperCase();
  if (!['A+','B+','C+'].includes(origGrade)) return {pass:false};

  // Note20 특수 처리
  if (/Note\s*20/i.test(rec.marketName)) {
    return { pass:true, reason:`Refurbish: Note20, Grade ${rec.grade}` };
  }
  ...
  if (rec.modelType === 'Bar') {
    ...
    if (N !== null) {
      const lo = N - 5, hi = N;
      if (rec.seriesNum >= lo && rec.seriesNum <= hi) {
        return { pass:true, reason:`Refurbish: Bar type, ${rec.series}${hi}~${rec.series}${lo}, Grade ${rec.grade}` };
      }
    }
  } else if (rec.modelType === 'FF') {
    const N = getNModel('Z');
    if (N !== null) {
      const lo = N - 2, hi = N - 1;
      if (rec.seriesNum >= lo && rec.seriesNum <= hi) {
        return { pass:true, reason:`Refurbish: FF type, FF${hi}~FF${lo}, Grade ${rec.grade}` };
      }
    }
  }
  return {pass:false};
